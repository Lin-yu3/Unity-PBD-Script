...
public class pbd06_withoutAD : MonoBehaviour
{
    //此為沒有使用 auto diff的版本
    ...
    void Start()
    {
        for (int i = 0; i < N; i++)
        {
            ...
            pos0[i] = sphere[i].transform.position;//只會改位置值,不回移動位置
        }
        genStretchBendingConstraint();
    }
    void Update()
    {
        frame++;//因為processing一次30fram,unity一次60frame
        if (frame % 2 == 0) return;//因此將unity執行速度減半
        solver();
        for (int i = 1; i < N; i++)
        {
    ...
    float d0 = 1.5f;//球之間的原始長度
    float Ci(int i, Vector3[] pos)//Ci計算stretch
    {
    ...
    float Cbendi(int i, Vector3[] pos)//計算bending
    {
    ...
    void solver()
    {
        Vector3 g = new Vector3(0, -0.98f / 20, 0);//由於unity的比例為processing的1/20倍
        for (int i = 1; i < N; i++)
        {
            v[i] += g;//Step (5) 還沒有乘上 delta T 及重量//step(6) Dammping沒做
            pos[i] = pos0[i] + v[i];//Step (7)
            v[i] *= 0.999999f;//消弱擺盪的力量
        }

        projectConstraints();//step(9)-(11)
